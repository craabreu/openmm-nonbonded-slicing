...
CudaFFT3DMany::CudaFFT3DMany(CudaContext& context, int xsize, int ysize, int zsize, int batch, bool realToComplex) :
...
void CudaFFT3DMany::execFFT(CudaArray& in, CudaArray& out, bool forward) {
...
int CudaFFT3DMany::findLegalDimension(int minimum) {
    if (minimum < 1)
        return 1;
    while (true) {
...
CUfunction CudaFFT3DMany::createKernel(int xsize, int ysize, int zsize, int batch, int& threads, int axis, bool forward, bool inputIsReal) {
...
//        maxThreads -= 64;
    ...
    CUmodule module = context.createModule(CudaPmeSlicingKernelSources::vectorOps+context.replaceStrings(CudaPmeSlicingKernelSources::fft, replacements));
