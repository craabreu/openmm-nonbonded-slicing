...
public:
    ...
    void initialize(OpenCLArray& pmeEnergyBuffer, OpenCLArray& sliceLambda, vector<string> requestedDerivs, OpenCLArray& sliceDerivIndices) {
        ...
        string source = cl.replaceStrings(CommonNonbondedSlicingKernelSources::slicedPmeAddEnergy, replacements);
...
void OpenCLCalcSlicedPmeForceKernel::initialize(const System& system, const SlicedPmeForce& force) {
    ...
    if (force.getIncludeDirectSpace()) {
        ...
        string source = cl.replaceStrings(CommonNonbondedSlicingKernelSources::coulomb, replacements);
    ...
    if (numExclusions > 0 && force.getIncludeDirectSpace()) {
       ...
       for (int k = 0; k < numExclusions; k++) {
        ...
        replacements["COMPUTE_DERIVATIVES"] = code.str();
        bonded->addInteraction(exclusionPairs, cl.replaceStrings(CommonNonbondedSlicingKernelSources::slicedPmeExclusions, replacements), force.getForceGroup());
    }
    ...
    if (numExceptions > 0 && force.getIncludeDirectSpace()) {
        ...
        replacements["COMPUTE_DERIVATIVES"] = code.str();
        bonded->addInteraction(exceptionPairs, cl.replaceStrings(CommonNonbondedSlicingKernelSources::slicedPmeExceptions, replacements), force.getForceGroup());
    }
    ...
    cl::Program program = cl.createProgram(CommonNonbondedSlicingKernelSources::slicedPmeParameters, paramsDefines);
...
double OpenCLCalcSlicedPmeForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy, bool includeDirect, bool includeReciprocal) {
    ...
    if (!hasInitializedKernel) {
        ...
        if (pmeGrid1.isInitialized()) {
            ...
            cl::Program program = cl.createProgram(realToFixedPoint+cl.replaceStrings(CommonNonbondedSlicingKernelSources::slicedPme, replacements), pmeDefines);
...
public:
    ...
    void initialize(OpenCLArray& pmeEnergyBuffer, OpenCLArray& ljpmeEnergyBuffer, OpenCLArray& sliceLambdas, vector<string> scalingParams, vector<mm_int2> sliceScalingParamDerivs) {
        ...
        if (cl.getContextIndex() == 0) {
            ...
            if (cl.getPlatformData().useCpuPme && !doLJPME && usePosqCharges) {
                ...
                try {
                    ...
                    cl::Program program = cl.createProgram(realToFixedPoint+CommonNonbondedSlicingKernelSources::pme, pmeDefines);
...
double OpenCLCalcSlicedNonbondedForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy, bool includeDirect, bool includeReciprocal) {
    ...
    if (!hasOffsets) {
        map<string, double>& energyParamDerivs = cl.getEnergyParamDerivWorkspace();
        for (int j = 0; j < numSubsets; j++) {
            mm_int2 indices = sliceScalingParamDerivsVec[j*(j+3)/2];
            int index = max(indices.x, indices.y);
            if (index != -1) {
