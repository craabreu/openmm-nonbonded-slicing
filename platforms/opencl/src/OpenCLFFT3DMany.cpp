...
OpenCLFFT3DMany::OpenCLFFT3DMany(OpenCLContext& context, int xsize, int ysize, int zsize, int batch, bool realToComplex) :
...
int OpenCLFFT3DMany::findLegalDimension(int minimum) {
    if (minimum < 1)
        return 1;
    while (true) {
...
cl::Kernel OpenCLFFT3DMany::createKernel(int xsize, int ysize, int zsize, int& threads, int axis, bool forward, bool inputIsReal) {
    ...
    while (true) {
        ...
        cl::Program program = context.createProgram(context.replaceStrings(OpenCLPmeSlicingKernelSources::fft, replacements));
