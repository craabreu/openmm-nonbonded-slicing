...
extern "C" __global__ void packForwardData(const real* __restrict__ in, real2* __restrict__ out) {
    ...
    for (int offset = 0; offset < TOTALSIZE; offset += ONESIZE) {
...
#if PACKED_AXIS == 0
            real2 value = make_real2(in[offset+2*x*YSIZE*ZSIZE+y*ZSIZE+z], in[offset+(2*x+1)*YSIZE*ZSIZE+y*ZSIZE+z]);
#elif PACKED_AXIS == 1
            real2 value = make_real2(in[offset+x*YSIZE*ZSIZE+2*y*ZSIZE+z], in[offset+x*YSIZE*ZSIZE+(2*y+1)*ZSIZE+z]);
#else
            real2 value = make_real2(in[offset+x*YSIZE*ZSIZE+y*ZSIZE+2*z], in[offset+x*YSIZE*ZSIZE+y*ZSIZE+(2*z+1)]);
#endif
...
#if PACKED_AXIS == 0
                if (x == 0)
                    out[offset+PACKED_XSIZE*YSIZE*outputZSize+yp*outputZSize+zp] = make_real2((z1.x-z1.y+z2.x-z2.y)/2, (-z1.x-z1.y+z2.x+z2.y)/2);
#elif PACKED_AXIS == 1
                if (y == 0)
                    out[offset+xp*YSIZE*outputZSize+PACKED_YSIZE*outputZSize+zp] = make_real2((z1.x-z1.y+z2.x-z2.y)/2, (-z1.x-z1.y+z2.x+z2.y)/2);
#else
                if (z == 0)
                    out[offset+xp*YSIZE*outputZSize+yp*outputZSize+PACKED_ZSIZE] = make_real2((z1.x-z1.y+z2.x-z2.y)/2, (-z1.x-z1.y+z2.x+z2.y)/2);
#endif
                else
                    out[offset+xp*YSIZE*outputZSize+yp*outputZSize+zp] = make_real2(output.x, -output.y);
            }
        }
    }
}
 ...
 */
static __inline__ __device__ real2 loadComplexValue(const real2* __restrict__ in, int x, int y, int z, int offset) {
    const int inputZSize = ZSIZE/2+1;
    if (z < inputZSize)
        return in[offset+x*YSIZE*inputZSize+y*inputZSize+z];
    int xp = (x == 0 ? 0 : XSIZE-x);
    int yp = (y == 0 ? 0 : YSIZE-y);
    real2 value = in[offset+xp*YSIZE*inputZSize+yp*inputZSize+(ZSIZE-z)];
...
extern "C" __global__ void packBackwardData(const real2* __restrict__ in, real2* __restrict__ out) {
    for (int offset = 0; offset < TOTALSIZE; offset += ONESIZE) {
        ...
        for (int index = blockIdx.x*blockDim.x+threadIdx.x; index < gridSize; index += blockDim.x*gridDim.x) {
            ...
            out[offset+x*PACKED_YSIZE*PACKED_ZSIZE+y*PACKED_ZSIZE+z] = make_real2(even.x-odd.y, even.y+odd.x);
        }
    }
}
