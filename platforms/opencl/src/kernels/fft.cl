...
__kernel void execFFT(__global const INPUT_TYPE* restrict in, __global OUTPUT_TYPE* restrict out, __local real2* restrict w,
    ...
    #if INPUT_IS_REAL
                    data0[z] = (real2) (in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+z], 0);
    #elif INPUT_IS_PACKED
                    data0[z] = loadComplexValue(in, x, y, z, offset);
    #else
                    data0[z] = in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+z];
    #endif
#else
                if (index < XSIZE*YSIZE)
    #if INPUT_IS_REAL
                    data0[get_local_id(0)] = (real2) (in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+get_local_id(0)%ZSIZE], 0);
    #elif INPUT_IS_PACKED
                    data0[get_local_id(0)] = loadComplexValue(in, x, y, get_local_id(0)%ZSIZE, offset);
    #else
                    data0[get_local_id(0)] = in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+get_local_id(0)%ZSIZE];
    #endif
#endif
#if OUTPUT_IS_PACKED
            }
