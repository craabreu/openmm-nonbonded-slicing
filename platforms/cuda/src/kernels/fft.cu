...
#if INPUT_IS_REAL
                    data0[i+block*ZSIZE] = make_real2(in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+i], 0);
#elif INPUT_IS_PACKED
                    data0[i+block*ZSIZE] = loadComplexValue(in, x, y, i, offset);
#else
                    data0[i+block*ZSIZE] = in[offset+x*(YSIZE*ZSIZE)+y*ZSIZE+i];
#endif
#if OUTPUT_IS_PACKED
            }
#endif
